...
PreserveElementColors.prototype.preserveColors = function() {
  console.log('_elementColors',this._elementColors);
  this._elementRegistry.forEach(element => {
    this._colorCache[element.id] = this._elementColors._get(element);

    this._elementColors._set(element, {
      stroke: '#000',
      fill: '#fff'
    });
  });
...
PreserveElementColors.prototype.resetColors = function() {
  this._elementRegistry.forEach(element => {
    ...
    if (cachedColors) {
      this._elementColors._set(element, cachedColors);
    }
  });
